...
if __name__ == '__main__':
    ...
    for dfGlobal, str in [(dfAgua, "Água")]:
        df = dfGlobal.copy()
        ...
        for clima, dfMerged in [("Com Clima", dfMerged.merge(dfClima, on="DATA", how="inner")), ("Sem Clima", dfMerged)]:
            ...
            for individuos, geracoes in [(100, 200), (200, 500), (500, 1000)]:
                ...
                for semente in sementes:
                    ...
                    t_inicio = time.time()
                    ga = GASVR(dfMerged, var, individuos, geracoes, 0.5, 12, semente)
                    best = ga.run()[0]
                    t_fim = time.time()
            ...
            for h_previsoes in [3, 6, 12]:
                ...
                global_treino_teste.to_csv(f"Resultados/SVR-Treino {str}.csv", index=False, sep=";")
                global_metricas.to_csv(f"Resultados/SVR-Métricas {str}.csv", index=False, sep=";")

    # plotTreinoTeste(dfResultado,
    #                 title=f"Consumo de {str} {h_previsoes} passos à frente - Treino/Teste SVR")
    # plotHistResiduos(dfResultado.tail(h_previsoes),
    #                  title=f"Histograma de Resíduos da Previsão do Consumo de {str} {h_previsoes} passos à frente - SVR")
    # print(dfResumo.head())
    #
    # dfPrevisao = prever(modelo, dfMerged, var, h_previsoes, best.n_lags)
    #
    # plotPrevisao(dfPrevisao, dfMerged,
    #              title=f"Previsão do Consumo de {str} {h_previsoes} passos à frente - SVR")
